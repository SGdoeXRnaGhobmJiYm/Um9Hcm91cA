            ...
            for archive in archives:
                try:
                    ...
                    for proxy in proxies:
                        if proxy["type"] == 4 and proxy["kind"] == 2 and proxy["timeout"] <= 10000:
...
open("proxy.txt", "w")
checkerproxy_net()
with open('proxy.txt') as result:
        uniqlines = set(result.readlines())
with open('proxy.txt', 'w') as rmdup:
        rmdup.writelines(set(uniqlines))
with open('proxy.txt') as r:
        u = sorted(r.readlines())
with open('proxy.txt', 'w') as dp:
        dp.writelines(sorted(u))
